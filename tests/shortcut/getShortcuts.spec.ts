...
describe("Create Shortcut", () => {
  ...
  createShortcut(data:{shortLink:"${shortLink}", description:"${description}", sourceURL: "${sourceURL}"}) {
	...
	it("Fetch all the shortcuts", async () => {
		const query = `{
			getShortcuts {
				id
				shortLink
		...
		const response = await request(server.app)
			...
			.set(headers)
			.send({ query })
			.then((resp) => resp.body.data.getShortcuts);

		expect(response[0].id).not.toBeUndefined();
		expect(response[0].shortLink).toEqual(shortcut.shortLink);
		expect(response[0].description).toEqual(shortcut.description);
		expect(response[0].sourceURL).toEqual(shortcut.sourceURL);
	});
});
